...
class ConsistencyDistillation(nn.Module):
    def __init__(
        ...
        if is_mcd:
            if is_consistency_distillation:
                self.forward = self.forward_mcd_x
            else:
    ...
    def invDDIM(self, z_t, z_tk, tk, t):
        alpha_tk, alpha_t, sigma_tk, sigma_t = self.get_alpha_sigma(tk, t, z_t)
        x = (z_t-((sigma_t/sigma_tk.clamp(min = 1e-8))*z_tk))/(alpha_t-(alpha_tk*(sigma_t/sigma_tk.clamp(min = 1e-8)))).clamp(min = 1e-8)
        return x
    ...
    def forward_mcd_x(self, txt_latent, mask, class_id, seq2seq_cond=None, seq2seq_mask=None, return_x_start=False, k=None, train_i=1, *args, **kwargs):
        ...
        #pred v
        v = (z_t - alpha_t * x) / sigma_t.clamp(min = 1e-8)
        v_inv = (z_s - alpha_s * x_inv) / sigma_s.clamp(min = 1e-8)

        #loss = self.loss_fn(x,x_inv, reduction="none")
        loss = self.loss_fn(v,v_inv, reduction="none")
